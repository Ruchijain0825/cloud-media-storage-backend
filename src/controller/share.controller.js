...
export const createShare = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Guest sharing currently supports files only
      ...
      if (!file.storage_key) {
        return res.status(500).json({
          success: false,
          message:
            "File storage information is not available",
        });
      }


      // ------------------------------------------------
      // GENERATE SUPABASE FILE URL
      // ------------------------------------------------
      ...
      const ownerEmail =
        ownerResult.rows[0]?.email || "Someone";
      ...
      return res.status(200).json({
        success: true,
        message:
          "File link sent successfully to the email address",
        guest: true,
      });
    }
...
export const getSharedWithMe = async (req, res) => {
  try {
    ...
    const shares =
      await getSharedWithMeModel(userId);
