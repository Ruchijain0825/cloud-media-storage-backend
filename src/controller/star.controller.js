...
export const addStar = async (req, res) => {
    try {
        ...
        const resource = await pool.query(`SELECT id FROM ${table} WHERE id = $1 AND owner_id = $2 AND is_deleted = false`, [resourceId, userId]);
        ...
        const result = await pool.query(`INSERT INTO stars(user_id, resource_type, resource_id) VALUES($1, $2, $3) ON CONFLICT(user_id, resource_type, resource_id) DO NOTHING RETURNING *`, [userId, resourceType, resourceId]);
...
export const removeStar = async (req, res) => {
    try {
        ...
        const result = await pool.query(`DELETE FROM stars WHERE user_id = $1 AND resource_type = $2 AND resource_id = $3 RETURNING *`, [userId, resourceType, resourceId]);
